...
public:
  ...
  static constexpr Metric<0, 0, 0, 0, 1, 0, 0> from_celcius(float celcius) {
    static_assert(s == 0 && m == 0 && kg == 0 && A == 0 && K == 1 && mol == 0 &&
                      cd == 0,
                  "only statically avaiable on Temperature");
...
// Volume literals
constexpr Volume operator""_m3(const long double v) { return Volume(v); }
constexpr Volume operator""_m3(const unsigned long long v) {
  return Volume(static_cast<float>(v));
}

constexpr Volume operator""_l(const long double v) { return Volume(v / 1e3); }
constexpr Volume operator""_l(const unsigned long long v) {
  return Volume(static_cast<float>(v) / 1e3);
}

constexpr Volume operator""_ml(const long double v) { return Volume(v / 1e6); }
...
constexpr Pressure operator""_bar(const long double v) { return Pressure(v); }
