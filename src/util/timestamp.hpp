...
public:
  static constexpr Duration from_s(float s) {
    return Duration(s * 1000000);
  }
  static constexpr Duration from_ms(uint32_t ms) {
    return Duration(ms * 1000);
  }

  static constexpr Duration from_us(uint32_t us) {
    return Duration(us);
  }
...
private:
  explicit constexpr Duration(uint32_t us) : m_us(us) {}
  uint32_t m_us;
};
